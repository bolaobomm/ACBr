     ...
     try
        if Cmd.Metodo = 'statusservico' then
         ...
         begin
          ...
          with ACBrCTe1.EventoCTe.Evento.Add do
           begin
             ...
             if Trim(infEvento.CNPJ) = '' then
                infEvento.CNPJ   := copy(DFeUtil.LimpaNumero(ACBrCTe1.WebServices.Consulta.CTeChave),7,14)
             else
              begin
             ...
             infEvento.cOrgao := StrToIntDef(copy(DFeUtil.LimpaNumero(ACBrCTe1.WebServices.Consulta.CTeChave),1,2),0);
          ...
          try
             ACBrCTe1.EnviarEventoCTe(StrToIntDef(Cmd.Params(3),1));
           ...
           if FileExists(Cmd.Params(0)) or FileExists(PathWithDelim(ACBrCTe1.Configuracoes.Geral.PathSalvar)+Cmd.Params(0)) then
            begin
              if FileExists(Cmd.Params(0)) then
                 ...
                 ACBrCTe1.EventoCTe.LerXML(PathWithDelim(ACBrCTe1.Configuracoes.Geral.PathSalvar)+Cmd.Params(0));
            end
           else
              raise Exception.Create('Arquivo '+Cmd.Params(0)+' não encontrado.');
...
//                          NFeRTXT.Free;
         ...
         begin
           ...
           ACBrCTe1.EnviarEventoCTe(ACBrCTe1.EventoCTe.idLote);
           ...
           if (Cmd.Params(2) = '1') then
            begin
              try
                 ...
                 ArqPDF := PathWithDelim(ACBrCTe1.DACTe.PathPDF)+ArqPDF+'-cte.pdf';
              except
                 raise Exception.Create('Erro ao criar o arquivo PDF');
              end;
            end;
         ...
         begin
           ...
           if FileExists(Cmd.Params(1)) or FileExists(PathWithDelim(ACBrCTe1.Configuracoes.Geral.PathSalvar)+Cmd.Params(1)) then
            begin
              if FileExists(Cmd.Params(1)) then
               ...
               begin
                 ACBrCTe1.EventoCTe.LerXML(PathWithDelim(ACBrCTe1.Configuracoes.Geral.PathSalvar)+Cmd.Params(1));
                 ArqEvento := PathWithDelim(ACBrCTe1.Configuracoes.Geral.PathSalvar)+Cmd.Params(1);
               end;
            end
           else
              raise Exception.Create('Arquivo '+Cmd.Params(1)+' não encontrado.');
           ...
           ACBrCTe1.Conhecimentos.Clear;
           if FileExists(Cmd.Params(2)) or FileExists(PathWithDelim(ACBrCTe1.Configuracoes.Geral.PathSalvar)+Cmd.Params(2)) then
            begin
              if FileExists(Cmd.Params(2)) then
               ...
               begin
                 ACBrCTe1.Conhecimentos.LoadFromFile(PathWithDelim(ACBrCTe1.Configuracoes.Geral.PathSalvar)+Cmd.Params(2));
                 ArqCTe := PathWithDelim(ACBrCTe1.Configuracoes.Geral.PathSalvar)+Cmd.Params(2);
               end;
            end
           else
            begin
       ...
       begin
          ...
          case Compl.Entrega.TipoData of
           ...
           tdNaData,tdAteData,tdApartirData:
              begin
                ...
                Compl.Entrega.comData.dProg := DFeUtil.StringToDate(INIRec.ReadString('compl','dProg','0'));
              end;
           tdNoPeriodo:
              begin
                Compl.Entrega.noPeriodo.tpPer := StrToTpDataPeriodo(ok,INIRec.ReadString('compl','tpPer','0'));
                Compl.Entrega.noPeriodo.dIni  := DFeUtil.StringToDate(INIRec.ReadString('compl','dIni','0'));
                Compl.Entrega.noPeriodo.dFim  := DFeUtil.StringToDate(INIRec.ReadString('compl','dFim','0'));
              end;
          end;
        ...
        {$ENDIF}
          ...
          {$IFDEF PL_200}
            ...
            infCTeNorm.Rodo.dPrev := DFeUtil.StringToDate(INIRec.ReadString( 'Rodo','dPrev','0'));
       ...
       {$ENDIF}
         ...
         while true do
          ...
          {$ENDIF}
             begin
               nDup  := sFim;
               dVenc := DFeUtil.StringToDate(INIRec.ReadString( sSecao,'dVenc','0'));
         ...
         if INIRec.ReadString( 'infCteSub','chCte','') <> '' then
          ...
          {$ENDIF}
             ...
             {$ENDIF}
               ...
               tomaICMS.refNF.dEmi   := DFeUtil.StringToDate(INIRec.ReadString( 'infCteSub','dEmi','0'));
         ...
         if INIRec.ReadString( 'infCteAnu','chCte','') <> '' then
          ...
          {$IFDEF PL_200}
            ...
            InfCTeAnu.dEmi  := DFeUtil.StringToDate(INIRec.ReadString( 'infCteAnu','dEmi','0'));
          {$ELSE}
