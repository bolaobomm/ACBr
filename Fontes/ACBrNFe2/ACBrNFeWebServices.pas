...
begin
  ...
  try
    ...
    for i:= 0 to TACBrNFe( FACBrNFe ).NotasFiscais.Count-1 do
    begin
      if pos(String(FNFeChave), TACBrNFe( FACBrNFe ).NotasFiscais.Items[i].NFe.infNFe.ID) > 0 then
      begin
        Atualiza := True;
...
begin
  inherited Create(AOwner);
  FNFeRetorno := TRetCancNFe.Create;

  FStatus  := stNFeCancelamento;
...
begin
  if DFeUtil.EstaVazio(AValue) then
     GerarException('Informar uma Justificativa para cancelar a Nota Fiscal Eletronica')
  else
    AValue := DFeUtil.TrataString(AValue);

  if Length(AValue) < 15 then
     GerarException('A Justificativa para Cancelamento da Nota Fiscal Eletronica deve ter no minimo 15 caracteres')
  else
    FJustificativa := Trim(AValue);
end;

function TNFeCancelamento.GerarPathPorCNPJ(ChaveNFe: String): String;
var
  CNPJ: String;
begin
  if FConfiguracoes.Arquivos.SepararPorCNPJ then
    CNPJ := NotaUtil.ExtraiCNPJChaveAcesso( ChaveNFe )
  else
    CNPJ := '';

  Result := FConfiguracoes.Arquivos.GetPathCan( CNPJ );
end;

procedure TNFeCancelamento.DefinirServicoEAction;
begin
  FServico    := CURL_WSDL+'NfeCancelamento2';
  FSoapAction := FServico;
end;

procedure TNFeCancelamento.DefinirDadosMsg;
var
  CancNFe: TcancNFe;
  OK : Boolean;
begin
  ...
  try
    CancNFe.chNFe   := FNFeChave;
    CancNFe.tpAmb   := TpcnTipoAmbiente(FConfiguracoes.WebServices.AmbienteCodigo-1);
    CancNFe.nProt   := FProtocolo;
    CancNFe.xJust   := FJustificativa;
    ...
    AssinarXML( CancNFe.Gerador.ArquivoFormatoXML,
                'Falha ao assinar Cancelamento Nota Fiscal Eletrônica '+LineBreak + FMsg);
    ...
    if not(NotaUtil.Valida(FDadosMsg, FMsg, TACBrNFe( FACBrNFe ).Configuracoes.Geral.PathSchemas,
      ...
      GerarException('Falha na validação dos dados do cancelamento '+LineBreak+FMsg)
  finally
    CancNFe.Free;
  end;
end;

procedure TNFeCancelamento.SalvarEnvio;
var
  aPath: String;
begin
  ...
  if FConfiguracoes.Arquivos.Salvar then
  begin
    aPath := GerarPathPorCNPJ( FNFeChave );
    FConfiguracoes.Geral.Save(GerarPrefixoArquivo+'-'+ArqEnv+'.xml', FDadosMsg, aPath);
  end;
end;


procedure TNFeCancelamento.SalvarResposta;
var
  aPath: String;
begin
  ...
  begin
    aPath := GerarPathPorCNPJ( FNFeChave );
    FConfiguracoes.Geral.Save(GerarPrefixoArquivo+'-'+ArqResp+'.xml', FRetWS, aPath);
  end;
end;


function TNFeCancelamento.GerarPrefixoArquivo: AnsiString;
begin
  Result := Trim(FNFeChave);
end;

function TNFeCancelamento.TratarResposta: Boolean;
var
  I: Integer;
  NomeArq: String;
  wProc: TStringList;
...
begin
  ...
  FxMotivo   := FNFeRetorno.xMotivo;
  FcUF       := FNFeRetorno.cUF;
  FDhRecbto  := FNFeRetorno.dhRecbto;
  Fprotocolo := FNFeRetorno.nProt;
  FMsg       := FNFeRetorno.XMotivo;

  Result := (FNFeRetorno.CStat in [101, 151, 155]);

  for I := 0 to TACBrNFe( FACBrNFe ).NotasFiscais.Count-1 do
  begin
    if pos(FNFeRetorno.chNFE, TACBrNFe( FACBrNFe ).NotasFiscais.Items[I].NFe.infNFe.ID) > 0  then
    begin
      if (FConfiguracoes.Geral.AtualizarXMLCancelado) then
      begin
        TACBrNFe( FACBrNFe ).NotasFiscais.Items[I].Msg                  := FNFeRetorno.xMotivo;
        TACBrNFe( FACBrNFe ).NotasFiscais.Items[I].NFe.procNFe.tpAmb    := FNFeRetorno.tpAmb;
        TACBrNFe( FACBrNFe ).NotasFiscais.Items[I].NFe.procNFe.verAplic := FNFeRetorno.verAplic;
        TACBrNFe( FACBrNFe ).NotasFiscais.Items[I].NFe.procNFe.chNFe    := FNFeRetorno.chNFe;
        TACBrNFe( FACBrNFe ).NotasFiscais.Items[I].NFe.procNFe.dhRecbto := FNFeRetorno.dhRecbto;
        TACBrNFe( FACBrNFe ).NotasFiscais.Items[I].NFe.procNFe.nProt    := FNFeRetorno.nProt;
        TACBrNFe( FACBrNFe ).NotasFiscais.Items[I].NFe.procNFe.cStat    := FNFeRetorno.cStat;
        TACBrNFe( FACBrNFe ).NotasFiscais.Items[I].NFe.procNFe.xMotivo  := FNFeRetorno.xMotivo;
      end;

      if Result then
      begin
        NomeArq := TACBrNFe( FACBrNFe ).NotasFiscais.Items[I].NomeArq;

        if FConfiguracoes.Arquivos.Salvar or DFeUtil.NaoEstaVazio(NomeArq) then
        begin
          if DFeUtil.EstaVazio( NomeArq ) then
          begin
            if FConfiguracoes.Arquivos.EmissaoPathNFe then
              Data := TACBrNFe( FACBrNFe ).NotasFiscais.Items[I].NFe.Ide.dEmi
            else
            ...
            NomeArq := PathWithDelim(FConfiguracoes.Arquivos.GetPathNFe( Data,
                         TACBrNFe( FACBrNFe ).NotasFiscais.Items[I].NFe.Emit.CNPJCPF))+
                       StringReplace(TACBrNFe( FACBrNFe ).NotasFiscais.Items[I].NFe.InfNFe.Id,'NFe','',[rfIgnoreCase])+'-nfe.xml'
          end;

          if not FileExists(NomeArq) or FConfiguracoes.Geral.AtualizarXMLCancelado then
            TACBrNFe( FACBrNFe ).NotasFiscais.Items[I].SaveToFile( NomeArq )
        end;
      end;
  ...
  begin
    ...
    if FConfiguracoes.Geral.Salvar then
       FConfiguracoes.Geral.Save(GerarPrefixoArquivo+'-ProcCancNFe.xml', FXML_ProcCancNFe);

    if FConfiguracoes.Arquivos.Salvar then
      FConfiguracoes.Geral.Save(GerarPrefixoArquivo+'-ProcCancNFe.xml', FXML_ProcCancNFe,
            GerarPathPorCNPJ( FNFeChave ) );
  end;
end;

function TNFeCancelamento.GerarMsgLog: AnsiString;
begin
  Result := //'Versão Leiaute : '+NFeRetorno.Versao+LineBreak+
            'Identificador : '+ FNFeRetorno.chNFE+LineBreak+
            'Ambiente : '+TpAmbToStr(FNFeRetorno.TpAmb)+LineBreak+
            'Versão Aplicativo : '+FNFeRetorno.verAplic+LineBreak+
            'Status Código : '+IntToStr(FNFeRetorno.cStat)+LineBreak+
            'Status Descrição : '+FNFeRetorno.xMotivo+LineBreak+
            'UF : '+CodigoParaUF(FNFeRetorno.cUF)+LineBreak+
            'Chave Acesso : '+FNFeRetorno.chNFE+LineBreak+
            'Recebimento : '+DFeUtil.SeSenao(FNFeRetorno.DhRecbto = 0, '', DateTimeToStr(FNFeRetorno.DhRecbto))+LineBreak+
            'Protocolo : '+FNFeRetorno.nProt+LineBreak;
end;
*)
...
function TNFeCartaCorrecao.GerarPathEvento: String;
begin
  if not FConfiguracoes.Arquivos.SalvarCCeCanEvento then
    Result := FConfiguracoes.Arquivos.GetPathCCe
  else
    Result := FConfiguracoes.Arquivos.GetPathEvento(teCCe);
end;

procedure TNFeCartaCorrecao.DefinirServicoEAction;
begin
  FServico    := CURL_WSDL+'RecepcaoEvento';
  FSoapAction := FServico;
end;

procedure TNFeCartaCorrecao.DefinirDadosMsg;
var
  CCeNFe: TCCeNFe;
  I, F: Integer;
  Eventos, Evento, Lote, EventosAssinados: String;
begin
  CCeNFe := TCCeNFe.Create;
  try
    CCeNFe.idLote := FidLote;

    for I := 0 to FCCe.Evento.Count-1 do
    begin
      with CCeNFe.Evento.Add do
      begin
        infEvento.cOrgao               := FCCe.Evento[I].InfEvento.cOrgao;
        infEvento.tpAmb                := TpcnTipoAmbiente(FConfiguracoes.WebServices.AmbienteCodigo-1);
        infEvento.CNPJ                 := FCCe.Evento[I].InfEvento.CNPJ;
        infEvento.chNFe                := FCCe.Evento[I].InfEvento.chNFe;
        infEvento.dhEvento             := FCCe.Evento[I].InfEvento.dhEvento;
        infEvento.tpEvento             := FCCe.Evento[I].InfEvento.tpEvento;
        infEvento.nSeqEvento           := FCCe.Evento[I].InfEvento.nSeqEvento;
        infEvento.versaoEvento         := FCCe.Evento[I].InfEvento.versaoEvento;
        infEvento.detEvento.versao     := FCCe.Evento[I].InfEvento.detEvento.versao;
        infEvento.detEvento.descEvento := FCCe.Evento[I].InfEvento.detEvento.descEvento;
        infEvento.detEvento.xCorrecao  := FCCe.Evento[I].InfEvento.detEvento.xCorrecao;
        infEvento.detEvento.xCondUso   := FCCe.Evento[I].InfEvento.detEvento.xCondUso;
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        AssinarXML( Evento, 'Falha ao assinar o Envio de Evento '+LineBreak+FMsg );
    ...
    if not NotaUtil.Valida(FDadosMsg, FMsg, TACBrNFe( FACBrNFe ).Configuracoes.Geral.PathSchemas,
                           FConfiguracoes.Geral.ModeloDF, FConfiguracoes.Geral.VersaoDF) then
      GerarException('Falha na validação dos dados da carta de correção '+LineBreak+FMsg);

    for I := 0 to FCCe.Evento.Count-1 do
      FCCe.Evento[i].InfEvento.id := CCeNFe.Evento[i].InfEvento.id;
  finally
    CCeNFe.Free;
  end;
end;

function TNFeCartaCorrecao.TratarResposta: Boolean;
var
  I, J: Integer;
  Leitor: TLeitor;
  wProc: TStringList;
...
procedure TNFeCartaCorrecao.SalvarEnvio;
begin
  inherited SalvarEnvio;
...
procedure TNFeCartaCorrecao.SalvarResposta;
begin
  inherited SalvarResposta;
...
function TNFeCartaCorrecao.GerarMsgLog: AnsiString;
begin
  Result := 'Ambiente : '+TpAmbToStr(FCCeRetorno.tpAmb)+LineBreak+
            'Versão Aplicativo : '+FCCeRetorno.verAplic+LineBreak+
            'Status Código : '+IntToStr(FCCeRetorno.cStat)+LineBreak+
            'Status Descrição : '+FCCeRetorno.xMotivo+LineBreak+
            'Recebimento : '+DFeUtil.SeSenao(FCCeRetorno.retEvento.Items[0].RetInfEvento.dhRegEvento = 0, '',
                                             DateTimeToStr(CCeRetorno.retEvento.Items[0].RetInfEvento.dhRegEvento));
end;

function TNFeCartaCorrecao.GerarPrefixoArquivo: AnsiString;
begin
  Result := IntToStr(FCCe.idLote);
end;
*)
{ TNFeEnvEvento }
...
begin
  ...
  FCancelamento.NFeChave      := FConsulta.FNFeChave;
  FCancelamento.Protocolo     := FConsulta.FProtocolo;
  FCancelamento.Justificativa := AJustificativa;
  FCancelamento.Executar;
end;
*)
